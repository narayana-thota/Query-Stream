...
const PodcastGenPage = ({ toggleSidebar }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (storedName === 'User' || storedName.includes('@')) {
        ...
        tempName = tempName.replace(/[0-9]/g, '');
        if (tempName) {
  ...
  return (
    <div className="min-h-screen bg-[#0A0D17] text-[#F9FAFB] font-sans selection:bg-[#7F5AF0]/30">
                  ...
                  <div
                    ...
                    className={`relative p-3 rounded-xl border cursor-pointer transition-all duration-200 flex flex-col justify-between ${
                      selectedVoice === voice.id
                        ? 'bg-[#7F5AF0]/10 border-2 border-[#7F5AF0] shadow-[0_0_15px_rgba(127,90,240,0.2)]' // Highlights selection
