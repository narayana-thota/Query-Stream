...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // 1. SMART NAME LOGIC (Hidden from UI)
    // Automatically creates a name from the email so the backend doesn't crash.
    // Example: "narayana123@gmail.com" -> "Narayana"
    let derivedName = "User";
    if (email.includes('@')) {
        const namePart = email.split('@')[0];
        // Remove numbers and special chars to get a clean name
        const cleanName = namePart.replace(/[0-9._]/g, ''); 
        
        if (cleanName.length > 0) {
            derivedName = cleanName.charAt(0).toUpperCase() + cleanName.slice(1);
        } else {
            // Fallback if email is just numbers
            derivedName = namePart; 
        }
    }

    try {
      // ✅ Sends the 'derivedName' to backend invisibly
    ...
    <div className="w-full min-h-screen bg-[#0A0D17] relative overflow-x-hidden flex flex-col md:flex-row">
      ...
      <div className="flex w-full md:w-1/2 min-h-[500px] md:h-auto md:min-h-screen flex-col justify-center items-center relative z-10 p-6 pt-0 md:p-12 md:pl-48">
        <div ref={formRef} className="w-full max-w-md mx-auto mt-8 md:mt-16 will-change-transform"> 
          ...
          <p className="text-lg md:text-xl text-[#94A3B8] mb-8 md:mb-12">Start your new journey with us.</p>
